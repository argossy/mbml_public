...
pairwise_association_test = function(mx2,cdataNames,edataNames,dataName='out',dir_out = 'output'
  ...
  ## cdataNames = cdataNames_RSA; edataNames = edataNames_mask;n_decimal = 3;methods = rep('lm',length(edataNames));
    ...
    for(i in 1:length(cdataNames)){
      ...
      if(class(mx2[,cdataName]) == 'numeric'){
	      if(var(mx2[,cdataName], na.rm = TRUE) !=0){
	        #abline(lm1)
          abline(lm(mx2[,edataName] ~ mx2[,cdataName]))
	      }
	    }
      ...
      # 	#var_X = c(0,var_X)
      #   var_X = apply(model_mat, 2, var, na.rm = TRUE)

      # } else{
      # 	#xx = mx2[idx_keep,cdataName]
      #   xx = model_mat[idx_keep,]
...
pairwise_association_test_lmm = function(mx2,cdataNames,edataNames, randomName, cvrtNames=''
  ...
  for(j in 1:length(edataNames)){
    for(i in 1:length(cdataNames)){
      ...
      lmm_anova <- anova(lmm_fit) #compute the ANOVA table of main effects
      lmm_ranova <- ranova(lmm_fit)#compute the ANOVA-Like Table for Random-Effects
      lmm_ranova$BIC <- -2*lmm_ranova$logLik+log(length(sum1$residuals))*lmm_ranova$npar
      ...
      # use the Satterthwaite approximation to approximate p-value and df
      ...
      print(kable(lmm_anova, digits = round(5),caption = paste('Type III ANOVA: ',edataNames[j], " VS ", substr(cdataNames[i],nchar(cdataNames[i])-40,nchar(cdataNames[i])),sep='')))
      print(kable(lmm_ranova, digits = round(5),caption = paste('Random Effects ANOVA: ',edataNames[j], " VS ", substr(cdataNames[i],nchar(cdataNames[i])-40,nchar(cdataNames[i])),sep='')))
...
plot_longitudinal = function(mx2,cdataNames,edataNames, timeName
  ...
  for(j in 1:length(edataNames)){
    #y_min = min(0,mx2[,edataNames[j]], na.rm= TRUE)
    #y_max = max(mx2[,edataNames[j]], na.rm = TRUE)
